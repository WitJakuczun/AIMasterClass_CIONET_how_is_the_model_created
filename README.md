...
- evaluate ML Models perfomance using selected experiment
...
The project uses the "Financial Sentiment Analysis" dataset from Kaggle. You can find the dataset [here](https://www.kaggle.com/datasets/sbhatti/financial-sentiment-analysis).
...
This command allows you to use a trained model to make predictions on new, unseen data.

```bash
uv run cli predict-new --model-path <path_to_model> --input-file <path_to_data> [--prediction-id <id>]
```

-   `--model-path`: Path to the directory containing the trained model.
-   `--input-file`: Path to the CSV file with the data to predict.
-   `--prediction-id`: (Optional) A unique ID for this prediction run. If not provided, a timestamp-based ID will be generated.

### `evaluate-prediction`

After making predictions, you can use this command to evaluate their accuracy against a ground truth file.

```bash
uv run cli evaluate-prediction --predictions-file <path_to_predictions> --ground-truth-file <path_to_ground_truth> [--display]
```

-   `--predictions-file`: Path to the CSV file containing the model's predictions.
-   `--ground-truth-file`: Path to the CSV file containing the correct labels.
-   `--display`: (Optional) If provided, the evaluation metrics will be printed to the console.

## Model Comparison

### `compare-models`

This command allows you to compare the performance of different models within a single experiment.

```bash
uv run cli compare-models --experiment-id <id> [--output-file <path>] [--run-type <type>]
```

-   `--experiment-id`: The ID of the experiment to compare models from.
-   `--output-file`: (Optional) Path to save the comparison report.
-   `--run-type`: (Optional) The type of run to compare: `backtesting` or `performance_estimation`.
...
3.  **Run the Model**
    ...
    That's it! The new model is now available. You can see it by runnin `uv run cli list-models` and use it in experiments with `--model-config-name bert-micro-base`.
...
-   `artifacts/`: Contains all outputs from experiments, such as data splits, predictions, and trained models.
    -   `predictions/`: Contains predictions made by the `predict-new` command.
    -   `metrics/`: Contains evaluation metrics generated by the `evaluate-prediction` command.
