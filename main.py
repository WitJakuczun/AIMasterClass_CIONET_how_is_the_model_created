...
def generate_splits_command(
    experiment_id: str = typer.Option(..., help="Unique ID for the experiment."),
    input_file: str = typer.Option(
        str(paths.all_train_csv), help="Path to the input CSV file."
    ),
    target_column: str = typer.Option("Sentiment", help="Column to stratify on."),
    test_size: float = typer.Option(
        0.2, help="Proportion of the dataset for the hold-out test set."
    ),
    backtesting_strategy: str = typer.Option(
        "cv", help="Backtesting strategy: 'cv' or 'train-val'."
    ),
    cv_folds: int = typer.Option(5, help="Number of folds for cross-validation."),
    backtesting_val_size: float = typer.Option(
        0.15, help="Validation set size for the backtesting train/val split."
    ),
    perf_estimation_val_size: float = typer.Option(
        0.1, help="Validation set size for the performance estimation split."
    ),
    final_model_val_size: float = typer.Option(
        0.1, help="Validation set size for the final model training split."
    ),
):
    app.generate_splits(
        experiment_id,
        ...
        cv_folds,
        backtesting_val_size,
        perf_estimation_val_size,
        final_model_val_size,
    )


@typer_app.command(name="run-backtesting")
def run_backtesting_command(
    experiment_id: str = typer.Option(..., help="Unique ID for the experiment."),
...
def estimate_performance_command(
    experiment_id: str = typer.Option(..., help="Unique ID for the experiment."),
    model_config_name: str = typer.Option(..., help="Name of the model config to use."),
):
...
def train_final_model_command(
    experiment_id: str = typer.Option(..., help="Unique ID for the experiment."),
    model_config_name: str = typer.Option(..., help="Name of the model config to use."),
    model_output_dir: str = typer.Option(
        str(paths.trained_models), help="Directory to save the final model."
    ),
):
...
def train_command(
    ...
    model_config_name: str = typer.Option(
        "gemma-3-1b-it", help="Name of the model config to use from models_config.py"
    ),
    model_output_dir: str = typer.Option(
        str(paths.trained_models), help="Directory to save the trained model."
    ),
):
    """
...
def predict_command(
    ...
    model_input_dir: str = typer.Option(
        str(paths.trained_models), help="Directory where the trained model is saved."
    ),
    prediction_output_dir: str = typer.Option(
        str(paths.predictions), help="Directory to save the predictions."
    ),
):
    """
...
def evaluate_command(
    ...
    prediction_input_dir: str = typer.Option(
        str(paths.predictions), help="Directory where the predictions are saved."
    ),
) -> dict:
    """
...
def run_experiment_command(
    experiment_id: str = typer.Option(..., help="Unique ID for the experiment."),
    model_config_name: str = typer.Option(
        "gemma-2b", help="Name of the model to use from models_config.py"
    ),
    model_output_dir: str = typer.Option(
        str(paths.trained_models), help="Directory to save the trained model."
    ),
    ...
    """
    app.run_experiment(
        experiment_id, model_config_name, model_output_dir, prediction_output_dir
    )


@typer_app.command(name="predict-new")
def predict_new_command(
    model_path: str = typer.Option(..., help="Path to the trained model."),
    input_file: str = typer.Option(
        ..., help="Path to the input CSV file for predictions."
    ),
...
def compare_models_command(
    ...
    run_type: str = typer.Option(
        None, help="Run type to compare: 'backtesting' or 'performance_estimation'."
    ),
):
    """
